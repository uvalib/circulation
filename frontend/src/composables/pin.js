...
export function usePinnable( pinClass, bodyClass, pinMargin ) {
   ...
   const scrolled = (() => {
      if ( window.scrollY <= toolbarTop.value ) {
         if ( toolbar.value.classList.contains("sticky") ) {
            ...
            let dts = document.getElementsByClassName(bodyClass)
            if ( dts ) {
               dts[0].style.top = `0px`
            }
         }
         ...
         if ( toolbar.value.classList.contains("sticky") == false ) {
            let dts = document.getElementsByClassName(bodyClass)
            if ( dts ) {
               dts[0].style.top = `${toolbarHeight.value+pinMargin}px`
            }
