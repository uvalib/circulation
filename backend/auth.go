...
func (svc *serviceContext) authenticate(c *gin.Context) {
	...
	if svc.DevAuthUser == "" {
		...
		if !strings.Contains(membershipStr, "lib-circulation-query") {
			log.Printf("ERROR: %s is not part of lib-circulation-query", computingID)
			c.Redirect(http.StatusFound, "/forbidden")
			return
		}
	}
	...
	c.SetCookie("cq_jwt", signedStr, 10, "/", "", false, false)
